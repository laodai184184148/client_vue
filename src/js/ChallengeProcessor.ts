...
export class ChallengeProcessor {
  private readonly challengeTask: string;
  challengeDetails: ChallengeDetails;
  private readonly verifyCallback: (success: boolean, remoteVerifier: RemoteVerifier) => void;
  private readonly endCallback: (success: boolean, challengeDetails: ChallengeDetails) => void;
  ...
  constructor(
    ...
    verifyCallback: (success: boolean, remoteVerifier: RemoteVerifier) => void,
    endCallback: (success: boolean, challengeDetails: ChallengeDetails) => void,
    ...
    if (this.challengeTask === "turn-face-left") {
      this.frameSave = this.challengeDetails.taskOrder["turn-face-left"];
    } else if (this.challengeTask === "turn-face-right") {
      this.frameSave = this.challengeDetails.taskOrder["turn-face-right"];
    } else
      this.frameSave = this.challengeDetails.taskOrder["keep-face-ahead"];

    this.endCallback = endCallback;
    this.helpMessageCallback = helpMessageCallback;
    this.helpAnimatonCallback = helpAnimatonCallback;
    console.log(this.challengeTask, this.frameSave);


    this.stateManager = new StateManager(this.challengeTask, this.challengeDetails);
    ...
    // new vertifier
    this.remoteVerifier = new RemoteVerifier(this.challengeDetails.id, this.videoElement);
    ...
    this.drawer = new Drawer(this.challengeDetails, this.canvasElement);
    this.drawer.drawFaceLine("fail",this.challengeTask);
    ...
    self.videoElement.addEventListener("loadedmetadata", function () {
      ...
      // setTimeout(() => {
      //   self.endCallback(false, self.challengeDetails);
    ...
    faceapi
      ...
      .then(
        ...
        function (result: faceapi.WithFaceLandmarks<{ detection: faceapi.FaceDetection }, faceapi.FaceLandmarks68>[]) {
          ...
          if (challengeProcessor.frameSave === 0) return;
          if (result) {
            if (result.length>0) console.log(result[0].landmarks.positions);
  ...
  static processDetectionResults(
    ...
    console.log(stateManagerOutput);
    ...
    if (stateManagerOutput.shouldSaveFrame && challengeProcessor.frameSave > 0) {
      --challengeProcessor.frameSave;
      challengeProcessor.drawer.drawFaceLine("success",challengeProcessor.challengeTask);
      console.log(challengeProcessor.challengeTask, challengeProcessor.frameSave);

      challengeProcessor.remoteVerifier.uploadFrame(challengeProcessor.challengeTask);
    } else challengeProcessor.drawer.drawFaceLine("fail",challengeProcessor.challengeTask);
    if (challengeProcessor.frameSave === 0) {
      console.log("Frame count", challengeProcessor.frameSave);
      
      challengeProcessor.endCallback(true, challengeProcessor.challengeDetails);
      if (challengeProcessor.challengeTask === "keep-face-ahead") {
        challengeProcessor.verifyCallback(true, challengeProcessor.remoteVerifier);
      }
